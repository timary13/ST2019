...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let length1 = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let length2 = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
...
function toNumber(value, def) {
    let parseResult = parseInt(value);
    if(!isNaN(parseResult) && parseResult.__proto__ == Number.prototype) {
        return value;
    }
    return def;
}
